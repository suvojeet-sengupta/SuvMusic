...
| Downloads | Shared `DownloadManager` plus shared Home/Search actions, status UI, and Downloads tab | Existing MediaStore/download queue remains protected; `DownloadsViewModel` consumes shared downloaded/progress flows while retaining legacy retry/batch operations | Linux downloader is wired into desktop Main, persists metadata atomically, exposes progress/status, and supports the shared Downloads tab |
