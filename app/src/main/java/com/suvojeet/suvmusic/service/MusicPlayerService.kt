...
class MusicPlayerService : MediaLibraryService() {
    ...
    private fun updateLikedState() {
        ...
        serviceScope.launch {
            ...
            val isLikedOnYT = try {
                // This might be slow if we fetch every time, 
                // but YouTubeRepository should have a cache
                youTubeRepository.getLikedMusic().any { it.id == currentSongId }
            } catch (e: Exception) {
                false
            }
            
            val history = listeningHistoryRepository.getHistoryForTimePeriod(365) // Just a way to get history
            // Actually, ListeningHistoryRepository should have a getSong(id) method
            // Let's use what's available
            
            isCurrentSongLiked = isLikedOnYT
            mediaLibrarySession?.let { it.setCustomLayout(getCustomLayout()) }
        }
    }
    ...
    override fun onCreate() {
        ...
        mediaLibrarySession = MediaLibrarySession.Builder(this, player, object : MediaLibrarySession.Callback {
            override fun onConnect(session: MediaSession, controller: MediaSession.ControllerInfo): MediaSession.ConnectionResult {
                ...
                // Add custom layout for notification/controls
    ...
    private inner class CustomNotificationProvider : MediaNotification.Provider {
        ...
        override fun createNotification(
            ...
            // Set ongoing flag to true to prevent dismissal
    ...
    override fun onUpdateNotification(session: MediaSession, startInForegroundRequired: Boolean) {
        // This ensures the notification stays in the foreground even when paused
        // but Media3 might still hide it if we don't handle it carefully.
